...
=====================================================================================
CS542 Project: TERC Windows on Earth

Set image model to inception_v3:
...
def add_final_training_ops(class_count, final_tensor_name, bottleneck_tensor, bottleneck_tensor_size):
  ...
  """
    ...
    # ====================================================================================
    # for multiclass classification, change activation to sigmoid
    ...
    with tf.name_scope('cross_entropy'):
        ...
        # ====================================================================================
        # for multiclass classification, change loss to sigmoid_cross_entropy
...
# =================================================================================
# add accuracy, precision, recall evaluation metrics for multiclass classification
def add_mul_class_evaluation_step(result_tensor, ground_truth_tensor):
    """Inserts the operations we need to evaluate the accuracy, precision and
	recall of our multo-class classification results.

	Args:
    	result_tensor: The new final node that produces results, the final_result layer
    	ground_truth_tensor: The node we feed ground truth data into.

  	Returns:
    	Tuple of (prediction, accuracy_step, precision_step, recall_step).
	"""
    with tf.name_scope('evaluation'):
        ...
        with tf.name_scope('precision'):
            precision, precision_step = tf.metrics.precision(
                labels=correct_prediction, predictions=prediction, name='precision')
        with tf.name_scope('recall'):
            recall, recall_step = tf.metrics.recall(
                labels=correct_prediction, predictions=prediction, name='recall')
...
def create_model_info(architecture):
  ...
  """
    ...
    if architecture == 'inception_v3':
        print('Retraining last layer on inception_v3')
...
def main(_):
    ...
    with tf.Session(graph=graph) as sess:
        ...
        prediction, accuracy_step, precision_step, recall_step = add_mul_class_evaluation_step(
        ...
        for i in range(FLAGS.how_many_training_steps):
            ...
            if (i % FLAGS.eval_step_interval) == 0 or is_last_step:
                ...
                # ====================================================================================
                train_accuracy, train_precision, train_recall, cross_entropy_value = sess.run(
                    [accuracy_step, precision_step, recall_step, cross_entropy],
                    feed_dict={bottleneck_input: train_bottlenecks,
                               ground_truth_input: train_ground_truth})
                tf.logging.info('%s: Step %d: Train accuracy = %.1f%%' %
                                (datetime.now(), i, train_accuracy * 100))
                ...
                tf.logging.info('%s: Step %d: Train recall = %.1f%%' %
                                (datetime.now(), i, train_recall * 100))
                tf.logging.info('%s: Step %d: Cross entropy = %f' %
                                (datetime.now(), i, cross_entropy_value))
        ...
        # ====================================================================================
        test_accuracy, test_precision, test_recall, predictions = sess.run(
            [accuracy_step, precision_step, recall_step, prediction],
            feed_dict={bottleneck_input: test_bottlenecks,
                       ground_truth_input: test_ground_truth})
        tf.logging.info('Final test accuracy = %.1f%% (N=%d)' %
                        (test_accuracy * 100, len(test_bottlenecks)))
        ...
        tf.logging.info('Final test recall = %.1f%% (N=%d)' %
                        (test_recall * 100, len(test_bottlenecks)))
        # ====================================================================================
